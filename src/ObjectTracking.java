...
public class ObjectTracking {
	// NOTE: Don't instantiate any openCV classes here - this will cause an exception.
	// Only do this after openCV has been initialized in main()
	...
	final static int MAX_OBJECT_AREA = (int) Math.round(FRAME_HEIGHT
			* FRAME_WIDTH / 1.5);
	// names that will appear at the top of each window
	final static String windowName = "Original Image";
	final static String windowName2 = "Thresholded Image";

	// erode with a 3px by 3px rectangle
	static Mat erodeElement = null;
	// dilate with larger element so make sure object is nicely visible
	static Mat dilateElement = null;
	...
	static void drawObject(int x, int y, Mat frame) {
		...
		if (y - 25 > 0)
			Imgproc.line(frame, currentPoint, new Point(x, y - 25),
					defaultColor, 2);
		else
			Imgproc.line(frame, currentPoint, new Point(x, 0), defaultColor, 2);
		if (y + 25 < FRAME_HEIGHT)
			Imgproc.line(frame, currentPoint, new Point(x, y + 25),
					defaultColor, 2);
		else
			Imgproc.line(frame, currentPoint, new Point(x, FRAME_HEIGHT),
					defaultColor, 2);
		if (x - 25 > 0)
			Imgproc.line(frame, currentPoint, new Point(x - 25, y),
					defaultColor, 2);
		else
			Imgproc.line(frame, currentPoint, new Point(0, y), defaultColor, 2);
		if (x + 25 < FRAME_WIDTH)
			Imgproc.line(frame, currentPoint, new Point(x + 25, y),
					defaultColor, 2);
		else
			Imgproc.line(frame, currentPoint, new Point(FRAME_WIDTH, y),
					defaultColor, 2);

		Imgproc.putText(frame, x + "," + y, new Point(x, y + 30), 1, 1,
				defaultColor, 2);
	...
	static void trackFilteredObject(int x, int y, Mat threshold, Mat cameraFeed) {
		...
		Imgproc.findContours(temp, contours, new Mat(), Imgproc.RETR_CCOMP,
		...
		if (contours.size() > 0) {
			...
			if (numObjects < MAX_NUM_OBJECTS) {
				for (MatOfPoint contour : contours) {
					...
					// we only want the object with the largest area so we save
					// a reference area each iteration and compare it to the 
				...
				if (objectFound) {
					Imgproc.putText(cameraFeed, "Tracking Object", new Point(0,
							50), 2, 1, new Scalar(0, 255, 0), 2);
	...
	public static void main(String[] argv) {
		...
		erodeElement = Imgproc.getStructuringElement(Imgproc.MORPH_RECT,
				new Size(3, 3));
		...
		capture.open(0);
		System.out.println("Original camera feed resolution");
		...
		// matrix. All of our operations will be performed within this loop
		while (true) {
			...
			// threshold matrix
			Core.inRange(HSV, new Scalar(H_MIN, S_MIN, V_MIN), 
					new Scalar(H_MAX, S_MAX, V_MAX), threshold);
